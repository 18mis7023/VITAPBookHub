...
class PythonRecyclerAdapter(val context: Context, val itemList:ArrayList<InterviewBook>) : RecyclerView.Adapter<PythonRecyclerAdapter.PythonViewHolder>() {
    class PythonViewHolder(view: View) : RecyclerView.ViewHolder(view) {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): PythonViewHolder {
        val view= LayoutInflater.from(parent.context).inflate(R.layout.recycler_interview_single_row,parent,false)

        return PythonViewHolder(view)
    }
    ...
    override fun onBindViewHolder(holder: PythonViewHolder, position: Int)  {
